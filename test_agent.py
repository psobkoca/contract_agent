...
from agent import ContractAgent, RedlineResponse
import prompts
...
def test_disclaimer_validation():
    ...
    valid_json = json.dumps({
        ...
        "suggested_redline": "The modified clause text.",
        "explanation": "Because of risk."
    })
    
    res = agent._parse_and_validate(valid_json)
    assert res.disclaimer == prompts.DISCLAIMER_TEXT
    
    # Invalid disclaimer should raise ValueError
    invalid_json = json.dumps({
        "disclaimer": "Invalid disclaimer text.",
        "suggested_redline": "The modified clause text.",
        "explanation": "Because of risk."
    })
...
def test_rag_fallback():
    ...
    assert isinstance(res, RedlineResponse)
    assert res.disclaimer == prompts.DISCLAIMER_TEXT
    assert "Fallback" in res.explanation
    assert len(res.suggested_redline) > 0
...
def test_react_tool_execution():
    ...
    final_json = json.dumps({
        "disclaimer": prompts.DISCLAIMER_TEXT,
        "suggested_redline": "Propose New York law.",
        "explanation": "Because we found NY precedents."
    })
    ...
    assert res.suggested_redline == "Propose New York law."
    assert res.disclaimer == prompts.DISCLAIMER_TEXT
...
def test_review_all_10_contracts():
    ...
    for filename in contract_files:
        ...
        for redline in redlines:
            assert redline["disclaimer"] == prompts.DISCLAIMER_TEXT, (
                f"Disclaimer mismatch in contract {contract_id}, clause {redline['clause_id']}"
            )
            assert len(redline["suggested_redline"]) > 0
            assert len(redline["explanation"]) > 0
